...
import { SIGNIN_PAGE, LEADER_BOARD_PAGE } from "../constant";
...
describe("Header", () => {
  ...
  it("highlights the correct menu item based on location", () => {
    ...
    render(
      <Provider store={store}>
        <MemoryRouter initialEntries={[LEADER_BOARD_PAGE]}>
          <Header />
        </MemoryRouter>
      </Provider>
    );

    expect(screen.getByTestId(LEADER_BOARD_PAGE)).toHaveClass("active");
  });

  it("dispatches resetUserData and navigates to SIGNIN_PAGE on logout", async () => {
    mockSessionStorage(true);
    ...
    await waitFor(() => {
      expect(mockNavigate).toHaveBeenCalledWith(SIGNIN_PAGE);
    });
  });
});
